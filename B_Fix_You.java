...
public class B_Fix_You
{
	...
	public static void main (String[] args) throws java.lang.Exception
	{
	    ...
	    while(t-->0)
	    {
         int n=sc.nextInt();
         int m=sc.nextInt();
         Map<Integer, Integer>al=new HashMap<>();
         for(int i=0; i<m; i++){
             int x=sc.nextInt();
             int y=sc.nextInt();
             al.put(x,y);
         }
         ArrayList<Integer> sortedKeys = new ArrayList<Integer>(al.keySet());
     ...
     for (int i=0; i<m && re>0; i++){
        long a= al.get(i),b=al.get(i);
        lc = lc*b/gcdLong(lc,b);
        rs += (re-n/lc)*a;
        re= n/lc;
    }
